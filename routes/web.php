...
Route::middleware(['web', 'auth:web'])->namespace('Hanoivip\Activity\Controllers')->prefix('user')->group(function () {
    ...
    Route::get('/activity/group', 'ActivityController@listGroup')->route('activity.group');
    // Liệt kê/cập nhật các hoạt động của người chơi đối với 1 group cụ thể
    Route::get('/activity/detail', 'ActivityController@detailGroup')->route('activity.detail');
    // Nhận phần thưởng đối với các hoạt động cụ thể
    Route::post('/activity/reward', 'ActivityController@getReward')->route('activity.reward');
    // Nhận cấu hình các hoạt động đối với 1 group cụ thể
    Route::get('/activity/info', 'ActivityController@getInfo')->route('activity.config');
});
// Admin Domain
Route::middleware(['web', 'admin'])->namespace('Hanoivip\Activity\Controllers')->prefix('ecmin')->group(function () {
    ...
    Route::get('/activity/new', 'ActivityController@newUI')->route('activity.cfg.new.ui');
